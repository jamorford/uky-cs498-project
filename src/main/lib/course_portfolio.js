...
module.exports.generateMustachePorfolio = async (portfolio_id) => {
  const raw_portfolio = await Portfolio.query()
      .eager({
        course: {
          department: true,
        },
  ...
  for (const i in raw_portfolio.outcomes) {
...
module.exports.generatePortfolio = async (department_id, course_number, instructor, semester, year, num_students, student_learning_outcomes, section) => {
  ...
  const portfolioSLOController = new PortfolioSLOController();
  const evaluationController = new EvaluationController();

  // Query info for the inputted course
  const course_exist = await courseController.getByAttributes(department_id, course_number);

  // Declare variables needed in main scope
  let course_id = -1;

  // Declare default expiration date (change manually when semesters change)
  const exp_date = Date.parse('January 1, 2020');

  // If course doesn't exist, make a new course
  if (course_exist.length == 0) {
    const course_result = await courseController.insert(department_id, course_number);
  ...
  const portfolio_exist = await portfolioController.getByAttributes(course_id, instructor, semester, num_students, section, year, exp_date);

  // If portfolio doesn't exist, make another one
  if (portfolio_exist.length == 0) {
    const portfolio_result = await portfolioController.insert(course_id, instructor, semester, num_students, section, year, exp_date);

    const portfolio_slo_result = await portfolioSLOController.insert(portfolio_result.id, student_learning_outcomes[0]);
    ...
    for (let i=0; i<artifact_ids.length; i++) {
      const student_indexes = random_students_subset.generateRandomStudentIndexes(num_students);
...
module.exports.new = async ({
  ...
  const portfolio = await this.generatePortfolio(department_id, course_number, instructor, semester, year, num_students, student_learning_outcomes, section);
